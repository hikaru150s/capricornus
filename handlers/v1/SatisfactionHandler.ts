...
import { Constraint, Goal, SatisfactionLog } from '../../entities';
...
router.get('/', asyncHandlers(async (req, res, next) => {
  try {
    ...
    let runner = getRepository(SatisfactionLog).createQueryBuilder('eval');
...
router.get('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const result = await getRepository(SatisfactionLog).findOneOrFail(req.params.id);
...
router.post('/', asyncHandlers(async (req, res, next) => {
  try {
    ...
    const r = await getRepository(SatisfactionLog).save(x);
...
router.put('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const x = await getRepository(SatisfactionLog).findOneOrFail(req.params.id);
    x.reviewer = req.body.reviewer;
    x.goal = getRepository(Goal).findOne(req.body.goalId);
    x.constraint = getRepository(Constraint).findOne(req.body.constraintId);
    x.value = req.body.value;
...
router.delete('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const x = await getRepository(SatisfactionLog).delete({ id: parseInt(req.params.id, 10) });
    if (x.affected > 0) {
      res.status(204);
    } else {
