...
import { Group, GroupScoring, GroupScoringLog } from '../../entities';
...
router.get('/', asyncHandlers(async (req, res, next) => {
  try {
    ...
    let runner = getRepository(GroupScoringLog).createQueryBuilder('eval');
...
router.get('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const result = await getRepository(GroupScoringLog).findOneOrFail(req.params.id);
...
router.post('/', asyncHandlers(async (req, res, next) => {
  try {
    const x = new GroupScoringLog();
    x.reviewer = req.body.reviewer;
    x.subject = getRepository(GroupScoring).findOne(req.body.subjectId);
    ...
    const r = await getRepository(GroupScoringLog).save(x);
...
router.put('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const x = await getRepository(GroupScoringLog).findOneOrFail(req.params.id);
    x.reviewer = req.body.reviewer;
    x.subject = getRepository(GroupScoring).findOne(req.body.subjectId);
    x.target = getRepository(Group).findOne(req.body.targetId);
    x.value = req.body.value;
...
router.delete('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const x = await getRepository(GroupScoringLog).delete({ id: parseInt(req.params.id, 10) });
    if (x.affected > 0) {
      res.status(204);
    } else {
