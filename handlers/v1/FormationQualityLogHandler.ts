...
import { Constraint, FormationQualityLog, Goal, Group } from '../../entities';
...
router.get('/', asyncHandlers(async (req, res, next) => {
  try {
    ...
    let runner = getRepository(FormationQualityLog).createQueryBuilder('eval');
...
router.get('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const result = await getRepository(FormationQualityLog).findOneOrFail(req.params.id);
...
router.post('/', asyncHandlers(async (req, res, next) => {
  try {
    const x = new FormationQualityLog();
    ...
    const r = await getRepository(FormationQualityLog).save(x);
...
router.put('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const x = await getRepository(FormationQualityLog).findOneOrFail(req.params.id);
    x.constraint = getRepository(Constraint).findOne(req.body.constraintId);
    x.goal = getRepository(Goal).findOne(req.body.goalId);
    x.reviewer = req.body.reviewer;
    x.target = getRepository(Group).findOne(req.body.targetId);
...
router.delete('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const x = await getRepository(FormationQualityLog).delete({ id: parseInt(req.params.id, 10) });
    if (x.affected > 0) {
      res.status(204);
    } else {
