...
import { GroupScoring } from '../../entities';
...
router.get('/', asyncHandlers(async (req, res, next) => {
  try {
    ...
    let runner = getRepository(GroupScoring).createQueryBuilder('eval');
...
router.get('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const result = await getRepository(GroupScoring).findOneOrFail(req.params.id);
...
router.post('/', asyncHandlers(async (req, res, next) => {
  try {
    ...
    const r = await getRepository(GroupScoring).save(x);
...
router.put('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const x = await getRepository(GroupScoring).findOneOrFail(req.params.id);
    x.name = req.body.name;
    x.start = req.body?.start ? new Date(req.body.start) : new Date();
...
router.delete('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const x = await getRepository(GroupScoring).delete({ id: parseInt(req.params.id, 10) });
    if (x.affected > 0) {
      res.status(204);
    } else {
