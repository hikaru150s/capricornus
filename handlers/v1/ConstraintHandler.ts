...
router.get('/', asyncHandlers(async (req, res, next) => {
  try {
    ...
    let runner = getRepository(Constraint).createQueryBuilder('eval');
...
router.get('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const result = await getRepository(Constraint).findOneOrFail(req.params.id);
...
router.post('/', asyncHandlers(async (req, res, next) => {
  try {
    ...
    const r = await getRepository(Constraint).save(x);
...
router.put('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const x = await getRepository(Constraint).findOneOrFail(req.params.id);
    x.name = req.body.name;
    x.description = req.body.description;
...
router.delete('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const x = await getRepository(Constraint).delete({ id: parseInt(req.params.id, 10) });
    if (x.affected > 0) {
      res.status(204);
    } else {
