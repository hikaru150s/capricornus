...
import { Evaluation, EvaluationLog, Question, User } from '../../entities';
...
router.get('/', asyncHandlers(async (req, res, next) => {
  try {
    ...
    let runner = getRepository(EvaluationLog).createQueryBuilder('eval');
...
router.get('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const result = await getRepository(EvaluationLog).findOneOrFail(req.params.id);
...
router.post('/', asyncHandlers(async (req, res, next) => {
  try {
    const x = new EvaluationLog();
    x.question = getRepository(Question).findOne(req.body.questionId);
    x.reviewer = req.body.reviewer;
    x.subject = getRepository(Evaluation).findOne(req.body.subjectId);
    ...
    const r = await getRepository(EvaluationLog).save(x);
...
router.put('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const x = await getRepository(EvaluationLog).findOneOrFail(req.params.id);
    x.question = getRepository(Question).findOne(req.body.questionId);
    x.reviewer = req.body.reviewer;
    x.subject = getRepository(Evaluation).findOne(req.body.subjectId);
    x.target = getRepository(User).findOne(req.body.targetId);
...
router.delete('/:id', asyncHandlers(async (req, res, next) => {
  try {
    const x = await getRepository(EvaluationLog).delete({ id: parseInt(req.params.id, 10) });
    if (x.affected > 0) {
      res.status(204);
    } else {
